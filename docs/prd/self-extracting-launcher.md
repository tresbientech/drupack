...
- Any change to release asset names, checksums or `release.json`, beyond the
  unversioned copy of each asset. That copy carries a second `checksums.txt`
  line and a `latest_url` field, so the README's download command can name
  `releases/latest/download`. It comes from a README decision taken after this
...
| `gzip -9`, as far as the standard library reaches | 116.5 MB | not measured |

The standard library carries no zstd and no xz, so DEFLATE is its only option
here. A 32 KB DEFLATE window cannot reach the repetition zstd finds with a
128 MB window. `gzip -9` produced 116,456,768 bytes, and Go's `compress/gzip` at
...
byte against the source sha256.

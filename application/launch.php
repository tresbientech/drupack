...
Options:
  ...
  --db-port PORT             Database port, 3306 for mysql and 5432 for pgsql
  --db-name NAME             Database name, for mysql and pgsql
  --db-user NAME             Database user, for mysql and pgsql
  --db-password PASSWORD     Database password, for mysql and pgsql
  --admin-user NAME          Administrator account for a first start, admin by default
  --admin-password PASSWORD  Administrator password for a first start, generated by default
...
function options(array $arguments, bool $drush, array $site): array
{
    $options = [
        ...
        'site-name' => environment('DRUPACK_SITE_NAME') ?? $site['site_name'],
        'no-browser' => null,
    ];
    $command = [];
    while ($arguments !== []) {
...
// Drupal CMS recipes enable these during site:install; the seed build removes them the
// same way. automatic_updates 4.1.0 stalls a cron request, and package_manager cannot
// write into the read-only application. A resumed modules step can run after an earlier one
...
const STEP_REPORTS = [
    ...
    'modules' => 'Removing modules the application cannot run',

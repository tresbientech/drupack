...
package main
import ("fmt"; "os")
func main() { fmt.Printf("args=%q env=%s data=%s", os.Args[1:], os.Getenv("DRUPACK_TEST_VALUE"), os.Getenv("DRUPACK_DATA_DIR")) }
'@ | Set-Content (Join-Path $temporary 'runtime.go')
  ...
  $expected = 'args=["alpha" "beta"] env=forwarded data=' + $data
  ...
  $warning = (& (Join-Path $temporary 'broken.exe') --data-dir $data retained 2>&1) -join "`n"
  ...
  $untrusted = (& (Join-Path $temporary 'broken.exe') --data-dir $data untrusted 2>&1) -join "`n"

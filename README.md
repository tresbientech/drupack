...
Make a folder for your site and download Drupack into it. The site's data lives beside the executable.
...
Drupack asks for an administrator name and password, installs your site, and opens `http://localhost:8080` in your browser. The first start on Windows also unpacks its runtime, which takes longer.
...
The terminal shows the address, where your site's data lives, the log file and how to stop. Caddy's messages, PHP warnings and PHP errors go to `data/logs/caddy.log`, so they stay out of your way.

Useful options:

- `--data-dir PATH` puts the site's data somewhere else. `DRUPACK_DATA_DIR` sets a default.
...
The `data` directory holds your database, uploads, private files, configuration exports and generated settings. Copy that directory to back your site up, with the site stopped. Keep the copy private: it holds your content and your site's secrets.

A newer Drupack keeps working with an existing `data` directory, which `tests/replacement.sh` checks on every release.
...
Use `--database pgsql` for PostgreSQL. `--db-port` defaults to `3306` for MySQL and `5432` for PostgreSQL. Later starts read the connection from your site's data.

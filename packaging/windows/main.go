...
func prepareRuntime(data string) (string, error) {
	root := filepath.Join(data, "runtime")
	...
	if err != nil {
		if activeErr == nil {
			fmt.Fprintf(os.Stderr, "Warning: could not read bundled runtime: %v. Using the previous runtime.\n", err)
			return active, nil
		}
		return "", err
	}
...
func bundledRuntime() (manifest, []byte, error) {
	...
	if runtimeManifest == "" || runtimeArchive == "" {
		return runtime, nil, errors.New("the executable does not contain a runtime archive")
	}
	if err := json.Unmarshal([]byte(runtimeManifest), &runtime); err != nil {
		return runtime, nil, fmt.Errorf("invalid runtime manifest: %w", err)
	}
	if runtime.Version == "" || len(runtime.Files) == 0 || filepath.Base(runtime.Version) != runtime.Version {
		return runtime, nil, errors.New("invalid runtime manifest")
	}
...
func extractArchive(destination string, archive []byte, runtime manifest) error {
	...
	for _, file := range reader.File {
		...
		output, err := os.OpenFile(path, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, os.FileMode(entry.Mode))
		if err == nil {
...
func storedManifest(directory string) (manifest, error) {
	...
	if runtime.Version == "" || len(runtime.Files) == 0 || filepath.Base(runtime.Version) != runtime.Version {
		return runtime, errors.New("invalid stored runtime manifest")
	}
	if err := validManifest(runtime); err != nil {
		return runtime, err
	}
...
func validManifest(runtime manifest) error {
	files := make(map[string]struct{}, len(runtime.Files))
	for _, file := range runtime.Files {
		if !safePath(file.Path) || file.SHA256 == "" {
			return errors.New("runtime manifest contains an unsafe path")
		}
		if _, found := files[file.Path]; found {
			return fmt.Errorf("runtime manifest declares a file twice: %s", file.Path)
		}
